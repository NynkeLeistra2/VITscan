...
- **Nog niet toegepast op de live database** — ik heb alleen de publishable/anon key, geen DB-wachtwoord of Supabase access token, en daarmee kan geen DDL uitgevoerd worden. Om dit af te ronden: plak de inhoud van `supabase/migrations/0001_init_schema.sql` in de Supabase SQL Editor (dashboard) en voer uit, óf geef een DB-connectiestring/access token door zodat ik het kan draaien.
